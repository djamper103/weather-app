...
export default function TimesOfDay({ state }) {
    ...
    return (
        <div className={modalViewType === 'detailPage' ? style.containerDetailPage : null}>
            {(state ? state : modalCurrentView).list[0].sys.pod.includes('d') ?
                <div className={modalViewType === 'modal' ? style.containerModal : style.container}>
                    ...
                    <div className={style.containerMain}>
                        <div>{`${new Date((state ? state : modalCurrentView).city.sunset * 1000).getHours()} : ${new Date((state ? state : modalCurrentView).city.sunset * 1000).getMinutes()}`}</div>
                        <div>{(state ? state : modalCurrentView).list[0].sys.pod.includes('d') ? "Sunset" : "Sunrise"}</div>
                    </div>
                </div>
                :
                <div className={modalViewType === 'modal' ? style.containerModal : style.container}>
                    ...
                    <div className={style.containerMain}>
                        <div>{`${new Date((state ? state : modalCurrentView).city.sunrise * 1000).getHours()} : ${new Date((state ? state : modalCurrentView).city.sunrise * 1000).getMinutes()}`}</div>
                        <div>{(state ? state : modalCurrentView).list[0].sys.pod.includes('d') ? "Sunset" : "Sunrise"}</div>
                    </div>
                </div>
            }
